// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef CPLRAMPINGRESPONSERESOURCE_H
#define CPLRAMPINGRESPONSERESOURCE_H

@class PBCodable, NSString;
@protocol NSCopying;



@interface CPLRampingResponseResource : PBCodable <NSCopying>

 {
    ? _has;
}


@property (nonatomic) BOOL allowed; // ivar: _allowed
@property (nonatomic) BOOL hasAllowed;
@property (readonly, nonatomic) BOOL hasResource;
@property (nonatomic) BOOL hasRetryAfterMillis;
@property (retain, nonatomic) NSString *resource; // ivar: _resource
@property (nonatomic) NSInteger retryAfterMillis; // ivar: _retryAfterMillis


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif