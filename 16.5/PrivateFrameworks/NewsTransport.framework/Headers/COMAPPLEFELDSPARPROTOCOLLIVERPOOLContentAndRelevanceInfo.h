// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef COMAPPLEFELDSPARPROTOCOLLIVERPOOLCONTENTANDRELEVANCEINFO_H
#define COMAPPLEFELDSPARPROTOCOLLIVERPOOLCONTENTANDRELEVANCEINFO_H

@class PBCodable;
@protocol NSCopying;



@interface COMAPPLEFELDSPARPROTOCOLLIVERPOOLContentAndRelevanceInfo : PBCodable <NSCopying>

 {
    ? _has;
}


@property (nonatomic) int contentRating; // ivar: _contentRating
@property (nonatomic) BOOL hasContentRating;
@property (nonatomic) BOOL hasRelevanceRating;
@property (nonatomic) int relevanceRating; // ivar: _relevanceRating


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)contentRatingAsString:(int)arg0 ;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)relevanceRatingAsString:(int)arg0 ;
-(int)StringAsContentRating:(id)arg0 ;
-(int)StringAsRelevanceRating:(id)arg0 ;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif