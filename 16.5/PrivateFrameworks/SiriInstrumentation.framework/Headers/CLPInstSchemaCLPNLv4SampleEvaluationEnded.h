// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef CLPINSTSCHEMACLPNLV4SAMPLEEVALUATIONENDED_H
#define CLPINSTSCHEMACLPNLV4SAMPLEEVALUATIONENDED_H

@class NSData;


#import "SISchemaInstrumentationMessage.h"

@interface CLPInstSchemaCLPNLv4SampleEvaluationEnded : SISchemaInstrumentationMessage {
    ? _has;
}


@property (nonatomic) BOOL evaluated; // ivar: _evaluated
@property (nonatomic) BOOL evaluationResult; // ivar: _evaluationResult
@property (nonatomic) BOOL evaluationResultOnTheFirstUserParse; // ivar: _evaluationResultOnTheFirstUserParse
@property (nonatomic) BOOL hasEvaluated;
@property (nonatomic) BOOL hasEvaluationResult;
@property (nonatomic) BOOL hasEvaluationResultOnTheFirstUserParse;
@property (nonatomic) BOOL hasMatchedIndex;
@property (nonatomic) BOOL hasUserProfileSandboxRequested;
@property (nonatomic) BOOL hasUserStatedTaskOnTheFirstUserParse;
@property (readonly, nonatomic) NSData *jsonData;
@property (nonatomic) int matchedIndex; // ivar: _matchedIndex
@property (nonatomic) BOOL userProfileSandboxRequested; // ivar: _userProfileSandboxRequested
@property (nonatomic) BOOL userStatedTaskOnTheFirstUserParse; // ivar: _userStatedTaskOnTheFirstUserParse


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)dictionaryRepresentation;
-(id)initWithDictionary:(id)arg0 ;
-(id)initWithJSON:(id)arg0 ;
-(id)suppressMessageUnderConditions;
-(void)deleteEvaluated;
-(void)deleteEvaluationResult;
-(void)deleteEvaluationResultOnTheFirstUserParse;
-(void)deleteMatchedIndex;
-(void)deleteUserProfileSandboxRequested;
-(void)deleteUserStatedTaskOnTheFirstUserParse;
-(void)writeTo:(id)arg0 ;


@end


#endif