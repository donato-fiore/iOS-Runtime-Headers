// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef GEOPUBLISHERATTRIBUTIONMANIFESTINDEX_H
#define GEOPUBLISHERATTRIBUTIONMANIFESTINDEX_H

@class PBCodable, PBDataReader, NSMutableArray, NSMutableDictionary;
@protocol NSCopying;



@interface GEOPublisherAttributionManifestIndex : PBCodable <NSCopying>

 {
    PBDataReader *_reader;
    NSMutableArray *_publisherAttributionFiles;
    NSMutableDictionary *_publisherToFileIndex;
    unsigned int _readerMarkPos;
    unsigned int _readerMarkLength;
    os_unfair_lock_s _readerLock;
    ? _flags;
}


@property (retain, nonatomic) NSMutableArray *publisherAttributionFiles;
@property (retain, nonatomic) NSMutableDictionary *publisherToFileIndex;


+(BOOL)isValid:(id)arg0 ;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)init;
-(id)initWithData:(id)arg0 ;
-(id)initWithDictionary:(id)arg0 ;
-(id)initWithJSON:(id)arg0 ;
-(id)jsonRepresentation;
-(id)publisherAttributionFilesAtIndex:(NSUInteger)arg0 ;
-(unsigned int)publisherToFileIndexForKey:(id)arg0 ;
-(void)addPublisherAttributionFiles:(id)arg0 ;
-(void)clearPublisherAttributionFiles;
-(void)clearPublisherToFileIndex;
-(void)copyTo:(id)arg0 ;
-(void)enumeratePublisherToFileIndexUsingBlock:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)readAll:(BOOL)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif