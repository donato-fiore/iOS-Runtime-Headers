...
@interface VMUSymbolStore : NSObject <NSSecureCoding, NSXMLParserDelegate>

 {
    ...
    unordered_map<std::string, std::unordered_set<unsigned long long>, std::hash<std::string>, std::equal_to<std::string>, std::allocator<std::pair<const std::string, std::unordered_set<unsigned long long>>>> _addressesGroupedByUuid;
...
-(BOOL)_readContentsOfDsymFile:(id)arg0 error:(*id)arg1 ;
