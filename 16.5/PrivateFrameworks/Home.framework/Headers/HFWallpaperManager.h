// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef HFWALLPAPERMANAGER_H
#define HFWALLPAPERMANAGER_H

@class NSString, NSUserDefaults;
@protocol CLLocationManagerDelegate, HFHomeManagerObserver, HFHomeObserver, HFNamedWallpaperSource;

#import <Foundation/Foundation.h>

#import "HFWallpaperFileManager.h"
#import "HFWallpaperImageCache.h"
#import "HFWallpaperLegacyFileManager.h"
#import "HFReaderWriterCache.h"

@interface HFWallpaperManager : NSObject <CLLocationManagerDelegate, HFHomeManagerObserver, HFHomeObserver>



@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (retain, nonatomic) HFWallpaperFileManager *fileManager; // ivar: _fileManager
@property (readonly) NSUInteger hash;
@property (retain, nonatomic) HFWallpaperImageCache *imageCache; // ivar: _imageCache
@property (retain, nonatomic) HFWallpaperLegacyFileManager *legacyFileManager; // ivar: _legacyFileManager
@property (retain, nonatomic) NSObject<HFNamedWallpaperSource> *namedWallpaperSource; // ivar: _namedWallpaperSource
@property (readonly) Class superclass;
@property (retain, nonatomic) NSUserDefaults *userDefaults; // ivar: _userDefaults
@property (readonly, nonatomic) BOOL wallpaperSourceRegistered; // ivar: _wallpaperSourceRegistered
@property (retain, nonatomic) HFReaderWriterCache *wallpapersCache; // ivar: _wallpapersCache


+(id)sharedInstance;
-(id)_cachedWallpaperForKey:(id)arg0 ;
-(id)_originalImageForWallpaper:(id)arg0 forProcessingGenerator:(BOOL)arg1 ;
-(id)_wallpaperForHomeKitObject:(id)arg0 createIfNeeded:(BOOL)arg1 ;
-(id)allNamedWallpaperThumbnailsForWallpaperCollectionType:(NSInteger)arg0 ;
-(id)allNamedWallpapersForWallpaperCollectionType:(NSInteger)arg0 ;
-(id)defaultUserSelectableWallpaperForHomeKitObject:(id)arg0 ;
-(id)defaultUserSelectableWallpaperForWallpaperCollectionType:(NSInteger)arg0 ;
-(id)defaultWallpaperForCollectionType:(NSInteger)arg0 ;
-(id)generateProcessedImageFromWallpaper:(id)arg0 originalImage:(id)arg1 ;
-(id)init;
-(id)processOriginalBlurredImageFromWallpaper:(id)arg0 originalImage:(id)arg1 ;
-(id)processOriginalImageFromWallpaper:(id)arg0 originalImage:(id)arg1 ;
-(id)wallpaperForHomeKitObject:(id)arg0 ;
-(id)wallpaperImageForWallpaper:(id)arg0 variant:(NSInteger)arg1 ;
-(id)wallpaperObjectForWallpaper:(id)arg0 currentUserInterfaceStyle:(NSInteger)arg1 ;
-(void)_dispatchWallpaperChangedforHomeKitObject:(id)arg0 ;
-(void)_logUserMetricsAfterSettingWallpaper;
-(void)_migrateIfNeeded;
-(void)_migrateToSunriseWallpaperIfNeeded;
-(void)_migrateWallpaperDict:(id)arg0 forHomeKitIdentifier:(id)arg1 ;
-(void)_preloadWallpaperForHomeKitObject:(id)arg0 ;
-(void)_pruneUnusedWallpapers;
-(void)_pruneUnusedWallpapersWithExistingHomeKitIdentifiers:(id)arg0 ;
-(void)_setWallpaper:(id)arg0 forHomeKitIdentifier:(id)arg1 ;
-(void)_setWallpaper:(id)arg0 image:(id)arg1 forHomeKitIdentifier:(id)arg2 ;
-(void)_setWallpaper:(id)arg0 image:(id)arg1 forHomeKitObject:(id)arg2 ;
-(void)home:(id)arg0 didRemoveRoom:(id)arg1 ;
-(void)homeManager:(id)arg0 didRemoveHome:(id)arg1 ;
-(void)homeManagerDidFinishInitialDatabaseLoad:(id)arg0 ;
-(void)preheatCache;
-(void)registerWallpaperSource:(id)arg0 processedSource:(id)arg1 ;
-(void)setWallpaper:(id)arg0 image:(id)arg1 forHomeKitObject:(id)arg2 ;


@end


#endif