// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef SXVIDEOPLAYERADSLOTVISIBILITYMONITOR_H
#define SXVIDEOPLAYERADSLOTVISIBILITYMONITOR_H

@class SVKeyValueObserver;


#import "SXVisibilityMonitor.h"

@interface SXVideoPlayerAdSlotVisibilityMonitor : SXVisibilityMonitor

@property (readonly, nonatomic) SVKeyValueObserver *observer; // ivar: _observer


-(id)initWithVideoPlayerViewController:(id)arg0 videoAdSlotVisiblePercentageProvider:(id)arg1 ;


@end


#endif