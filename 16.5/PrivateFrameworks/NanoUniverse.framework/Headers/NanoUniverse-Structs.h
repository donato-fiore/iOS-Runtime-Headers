...
typedef struct NUNIAegirPipelineConstants {
} // Error Processing Struct Fields
...
typedef struct NUNIAegirRenderUniforms {
    ...
    ? sphere;
    ? lightDirection;
    ? lookModelInverseMatrixFull;
    ? lookModelPositionFull;
    float lookModelInverseMatrix;
    float lookModelPosition;
    float viewModelInverseMatrix;
    float viewModelMatrix;
    float projectionViewModelMatrix;
    float offset;
    float aspect;
    float invResolution;
    float nearPlane;
