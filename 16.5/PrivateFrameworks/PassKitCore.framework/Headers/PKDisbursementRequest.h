// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef PKDISBURSEMENTREQUEST_H
#define PKDISBURSEMENTREQUEST_H

@class NSDecimalNumber, NSString, NSArray;
@protocol NSSecureCoding;

#import <Foundation/Foundation.h>


@interface PKDisbursementRequest : NSObject <NSSecureCoding>



@property (copy, nonatomic) NSDecimalNumber *amount; // ivar: _amount
@property (copy, nonatomic) NSString *countryCode; // ivar: _countryCode
@property (copy, nonatomic) NSString *currencyCode; // ivar: _currencyCode
@property (nonatomic) NSInteger requestSchedule; // ivar: _requestSchedule
@property (copy, nonatomic) NSArray *summaryItems; // ivar: _summaryItems


+(BOOL)supportsSecureCoding;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)isEqualToDisbursementRequest:(id)arg0 ;
-(id)initWithCoder:(id)arg0 ;
-(void)encodeWithCoder:(id)arg0 ;


@end


#endif