// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef NPKPROTOSTANDALONEPAYMENTCREDENTIAL_H
#define NPKPROTOSTANDALONEPAYMENTCREDENTIAL_H

@class PBCodable, NSString;
@protocol NSCopying;



@interface NPKProtoStandalonePaymentCredential : PBCodable <NSCopying>



@property (retain, nonatomic) NSString *expiration; // ivar: _expiration
@property (readonly, nonatomic) BOOL hasExpiration;
@property (readonly, nonatomic) BOOL hasLongDescription;
@property (readonly, nonatomic) BOOL hasSanitizedPrimaryAccountNumber;
@property (retain, nonatomic) NSString *longDescription; // ivar: _longDescription
@property (retain, nonatomic) NSString *sanitizedPrimaryAccountNumber; // ivar: _sanitizedPrimaryAccountNumber


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif