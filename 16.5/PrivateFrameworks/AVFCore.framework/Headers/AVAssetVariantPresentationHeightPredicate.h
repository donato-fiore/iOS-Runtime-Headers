// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef AVASSETVARIANTPRESENTATIONHEIGHTPREDICATE_H
#define AVASSETVARIANTPRESENTATIONHEIGHTPREDICATE_H

@class NSPredicate, NSPredicateOperator;



@interface AVAssetVariantPresentationHeightPredicate : NSPredicate {
    CGFloat _height;
    NSPredicateOperator *_operator;
}




+(BOOL)supportsSecureCoding;
-(BOOL)evaluateWithObject:(id)arg0 substitutionVariables:(id)arg1 ;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)initWithCoder:(id)arg0 ;
-(id)initWithPresentationHeight:(CGFloat)arg0 operatorType:(NSUInteger)arg1 ;
-(id)predicateFormat;
-(void)acceptVisitor:(id)arg0 flags:(NSUInteger)arg1 ;
-(void)dealloc;
-(void)encodeWithCoder:(id)arg0 ;


@end


#endif