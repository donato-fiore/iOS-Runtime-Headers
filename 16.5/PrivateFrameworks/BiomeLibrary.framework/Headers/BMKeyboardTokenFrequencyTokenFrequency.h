// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef BMKEYBOARDTOKENFREQUENCYTOKENFREQUENCY_H
#define BMKEYBOARDTOKENFREQUENCYTOKENFREQUENCY_H

@class BMEventBase, NSString;
@protocol BMStoreData;



@interface BMKeyboardTokenFrequencyTokenFrequency : BMEventBase <BMStoreData>



@property (readonly, nonatomic) int count; // ivar: _count
@property (readonly, nonatomic) unsigned int dataVersion;
@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (nonatomic) BOOL hasCount; // ivar: _hasCount
@property (readonly) NSUInteger hash;
@property (readonly) Class superclass;
@property (readonly, nonatomic) NSString *token; // ivar: _token


+(id)columns;
+(id)eventWithData:(id)arg0 dataVersion:(unsigned int)arg1 ;
+(id)validKeyPaths;
-(BOOL)isEqual:(id)arg0 ;
-(id)initByReadFrom:(id)arg0 ;
-(id)initWithJSONDictionary:(id)arg0 error:(*id)arg1 ;
-(id)initWithToken:(id)arg0 count:(id)arg1 ;
-(id)jsonDictionary;
-(id)serialize;
-(void)writeTo:(id)arg0 ;


@end


#endif