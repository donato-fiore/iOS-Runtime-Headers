typedef struct _NSZone {
} // Error Processing Struct Fields

typedef struct ANENotificationMessageStruct {
    int field0;
    int field1;
    void field2;
    void field3;
    unsigned int field4;
} ANENotificationMessageStruct;

typedef struct os_unfair_lock_s {
    unsigned int _os_unfair_lock_opaque;
} os_unfair_lock_s;

typedef struct __IOSurface {
} // Error Processing Struct Fields

typedef struct ANEBufferStruct {
    __IOSurface field0;
    unsigned int field1;
    int field2;
    int field3;
    unsigned int field4;
} ANEBufferStruct;

typedef struct ANEMemoryMappingParamsStruct {
    ANEBufferStruct field0;
    NSUInteger field1;
    unsigned int field2;
    unsigned int field3;
    NSUInteger field4;
} ANEMemoryMappingParamsStruct;

typedef struct VMData {
} // Error Processing Struct Fields

typedef struct VirtANEModel {
    unsigned int field0;
    NSInteger field1;
    unsigned int field2;
    unsigned int field3;
    unsigned int field4;
    unsigned int field5;
    NSUInteger field6;
    NSUInteger field7;
    NSUInteger field8;
    NSUInteger field9;
    unsigned int field10;
    NSUInteger field11;
    unsigned int field12;
    NSUInteger field13;
    NSUInteger field14;
    NSUInteger field15;
    NSUInteger field16;
    char field17;
    unsigned char field18;
    unsigned int field19;
    NSUInteger field20;
    unsigned int field21;
    unsigned int field22;
    unsigned int field23;
    NSUInteger field24;
    unsigned int field25;
    unsigned int field26;
    NSUInteger field27;
    unsigned int field28;
    NSUInteger field29;
    unsigned int field30;
    unsigned int field31;
    unsigned int field32;
    unsigned int field33;
    unsigned int field34;
    NSUInteger field35;
    NSUInteger field36;
    unsigned int field37;
    unsigned int field38;
    unsigned int field39;
    unsigned int field40;
    unsigned int field41;
    unsigned int field42;
    unsigned int field43;
    unsigned int field44;
    NSUInteger field45;
    NSInteger field46;
    unsigned int field47;
    unsigned int field48;
    NSUInteger field49;
    unsigned int field50;
    NSUInteger field51;
    unsigned int field52;
    NSUInteger field53;
    unsigned int field54;
    NSUInteger field55;
    unsigned int field56;
    NSUInteger field57;
    unsigned int field58;
    NSUInteger field59;
    unsigned int field60;
    NSUInteger field61;
} VirtANEModel;

typedef struct ANEDeviceStruct {
    void field0;
    void field1;
    void field2;
    unsigned char field3;
    int field4;
} ANEDeviceStruct;

