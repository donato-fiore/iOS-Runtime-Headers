// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef SNSOUNDPRINTASMOKEALARMMODELINPUT_H
#define SNSOUNDPRINTASMOKEALARMMODELINPUT_H

@class MLMultiArray, NSSet;
@protocol MLFeatureProvider;

#import <Foundation/Foundation.h>


@interface SNSoundPrintASmokeAlarmModelInput : NSObject <MLFeatureProvider>



@property (retain, nonatomic) MLMultiArray *detectedHistoryIn; // ivar: _detectedHistoryIn
@property (readonly, nonatomic) NSSet *featureNames;
@property (retain, nonatomic) MLMultiArray *input1; // ivar: _input1
@property (retain, nonatomic) MLMultiArray *stateIn; // ivar: _stateIn
@property (retain, nonatomic) MLMultiArray *thresholdedHistoryIn; // ivar: _thresholdedHistoryIn


-(id)featureValueForName:(id)arg0 ;
-(id)initWithInput1:(id)arg0 ;
-(id)initWithInput1:(id)arg0 stateIn:(id)arg1 thresholdedHistoryIn:(id)arg2 detectedHistoryIn:(id)arg3 ;


@end


#endif