// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.5.0 | SDK: 16.5.0


#ifndef _GCEXTENDEDMFICONTROLLERPROFILE_H
#define _GCEXTENDEDMFICONTROLLERPROFILE_H

@class NSString;
@protocol _GCControllerProfile;

#import <Foundation/Foundation.h>


@interface _GCExtendedMFiControllerProfile : NSObject <_GCControllerProfile>



@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (readonly) NSUInteger hash;
@property (readonly) Class superclass;


+(BOOL)logicalDeviceIsSonyBackbone:(id)arg0 ;
+(BOOL)physicalDeviceUsesACHomeForMenu:(id)arg0 ;
+(NSInteger)physicalDeviceGetIndicatedPlayerIndex:(id)arg0 ;
+(id)deviceManager;
+(id)logicalDevice:(id)arg0 makeControllerPhysicalInputProfileWithIdentifier:(id)arg1 ;
+(id)logicalDeviceControllerProductCategory:(id)arg0 ;
+(void)determineCapabilitiesWithServiceInfo:(id)arg0 initInfo:(struct ? *)arg1 ;
+(void)deviceManager:(id)arg0 prepareLogicalDevice:(id)arg1 ;
+(void)deviceManager:(id)arg0 willPublishPhysicalDevice:(id)arg1 ;
+(void)physicalDevice:(id)arg0 setIndicatedPlayerIndex:(NSInteger)arg1 ;
+(void)populateInitInfo:(struct ? *)arg0 forLogicalDevice:(id)arg1 ;


@end


#endif