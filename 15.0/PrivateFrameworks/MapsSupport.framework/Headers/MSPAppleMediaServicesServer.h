// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef MSPAPPLEMEDIASERVICESSERVER_H
#define MSPAPPLEMEDIASERVICESSERVER_H

@class NSMutableSet, NSString, NSXPCListener;
@protocol NSXPCListenerDelegate, GEOAppleMediaServicesProtocol;

#import <Foundation/Foundation.h>


@interface MSPAppleMediaServicesServer : NSObject <NSXPCListenerDelegate, GEOAppleMediaServicesProtocol>



@property (retain, nonatomic) NSMutableSet *connections; // ivar: _connections
@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (readonly) NSUInteger hash;
@property (retain, nonatomic) NSXPCListener *listener; // ivar: _listener
@property (readonly) Class superclass;


-(BOOL)listener:(id)arg0 shouldAcceptNewConnection:(id)arg1 ;
-(NSInteger)AMSMediaTaskTypeFromGEOAppleMediaServicesMediaType:(NSInteger)arg0 ;
-(id)additionalParameterWithType:(NSInteger)arg0 ;
-(id)init;
-(void)cleanConnections;
-(void)dealloc;
-(void)mediaResultWithIdentifiers:(id)arg0 bundleIdentifiers:(id)arg1 type:(NSInteger)arg2 artworkSize:(struct CGSize )arg3 screenScale:(CGFloat)arg4 completion:(id)arg5 ;


@end


#endif