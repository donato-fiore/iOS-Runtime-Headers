// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef ICATTACHMENTSYSTEMPAPERACTIVITYITEMSOURCE_H
#define ICATTACHMENTSYSTEMPAPERACTIVITYITEMSOURCE_H



#import "ICAttachmentActivityItemSource.h"

@interface ICAttachmentSystemPaperActivityItemSource : ICAttachmentActivityItemSource



-(id)activityViewController:(id)arg0 dataTypeIdentifierForActivityType:(id)arg1 ;
-(id)activityViewController:(id)arg0 itemForActivityType:(id)arg1 ;
-(id)activityViewControllerPlaceholderItem:(id)arg0 ;
-(id)attachmentTypeUTI;


@end


#endif