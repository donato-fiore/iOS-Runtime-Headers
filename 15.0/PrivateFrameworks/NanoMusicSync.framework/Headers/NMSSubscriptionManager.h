// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef NMSSUBSCRIPTIONMANAGER_H
#define NMSSUBSCRIPTIONMANAGER_H

@class ACAccount, NSString;
@protocol ICEnvironmentMonitorObserver, OS_dispatch_queue;

#import <Foundation/Foundation.h>


@interface NMSSubscriptionManager : NSObject <ICEnvironmentMonitorObserver>

 {
    NSObject<OS_dispatch_queue> *_queue;
    NSInteger _icStatusType;
    ACAccount *_activeAccount;
    NSUInteger _subscriptionStatus;
    NSUInteger _subscriptionSupport;
    NSUInteger _failedGetSubscriptionStatusAttempts;
}


@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (readonly) NSUInteger hash;
@property (readonly, nonatomic) NSUInteger subscriptionStatus;
@property (readonly, nonatomic) NSUInteger subscriptionSupport;
@property (readonly) Class superclass;


+(id)sharedManager;
-(NSUInteger)_statusWithActiveAccount:(id)arg0 icStatusType:(NSInteger)arg1 ;
-(id)init;
-(void)_handleAccountStoreDidChangeNotification:(id)arg0 ;
-(void)_handleICSubscriptionStatusDidChangeNotification:(id)arg0 ;
-(void)_updateActiveAccount;
-(void)_updateICSubscriptionStatusType;
-(void)_updateSubscriptionStatus;
-(void)_updateSubscriptionSupport;
-(void)dealloc;
-(void)environmentMonitorDidChangeNetworkReachability:(id)arg0 ;


@end


#endif