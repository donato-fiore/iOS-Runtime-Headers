// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef TSCHPRESETIMAGERSTACKEDCOLUMN_H
#define TSCHPRESETIMAGERSTACKEDCOLUMN_H



#import "TSCHPresetImagerBarFamily.h"

@interface TSCHPresetImagerStackedColumn : TSCHPresetImagerBarFamily



-(id)p_chartType;
-(struct CGRect )p_stackRectWithRects:(struct CGRect *)arg0 atIndex:(NSUInteger)arg1 ;
-(void)p_drawShadowedContentIntoContext:(struct CGContext *)arg0 size:(struct CGSize )arg1 contentsScale:(CGFloat)arg2 preset:(id)arg3 target:(int)arg4 shouldCache:(*BOOL)arg5 ;


@end


#endif