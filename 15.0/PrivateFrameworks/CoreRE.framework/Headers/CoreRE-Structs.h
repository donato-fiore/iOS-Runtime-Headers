...
typedef struct FixedInlineArray<unsigned char, 24> {
    unsigned char field0;
} FixedInlineArray<unsigned char, 24>;
...
typedef struct MeshPart {
    ...
    FixedInlineArray<unsigned char, 24> field7;
    StringID field8;
    unsigned int field9;
...
typedef struct array<std::vector<float>, 2> {
    vector<float, std::allocator<float>> __elems_;
} array<std::vector<float>, 2>;
