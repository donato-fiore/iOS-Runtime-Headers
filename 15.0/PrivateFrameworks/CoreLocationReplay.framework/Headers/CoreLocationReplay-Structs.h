...
typedef struct VioData {
    ...
    MatrixMxNT<3, 1, double> v_vio;
    MatrixMxNT<3, 3, double> v_vio_cov;
    BOOL is_delta_p_vio_valid;
    MatrixMxNT<3, 1, double> delta_p_vio;
    MatrixMxNT<3, 3, double> delta_p_vio_cov;
...
typedef struct QuaternionT<double> {
    ...
    array<double, 4> data_;
    bitset<4> visited_;
} QuaternionT<double>;

typedef struct array<double, 9> {
    CGFloat __elems_;
} array<double, 9>;

typedef struct bitset<9> {
    NSUInteger __first_;
} bitset<9>;

typedef struct MatrixMxNT<3, 3, double> {
    unk _vptr$MatrixMxNT;
    array<double, 9> data_;
    bitset<9> visited_;
} MatrixMxNT<3, 3, double>;

typedef struct MatrixMxNT<4, 1, double> {
    unk _vptr$MatrixMxNT;
    array<double, 4> data_;
    bitset<4> visited_;
} MatrixMxNT<4, 1, double>;

typedef struct array<double, 3> {
    CGFloat __elems_;
} array<double, 3>;

typedef struct bitset<3> {
    NSUInteger __first_;
} bitset<3>;

typedef struct MatrixMxNT<3, 1, double> {
    unk _vptr$MatrixMxNT;
    array<double, 3> data_;
    bitset<3> visited_;
} MatrixMxNT<3, 1, double>;

typedef struct array<double, 36> {
    CGFloat __elems_;
} array<double, 36>;

typedef struct bitset<36> {
    NSUInteger __first_;
} bitset<36>;

typedef struct MatrixMxNT<6, 6, double> {
    unk _vptr$MatrixMxNT;
    array<double, 36> data_;
    bitset<36> visited_;
} MatrixMxNT<6, 6, double>;

typedef struct DevicePose {
    BOOL is_deterministic_;
    QuaternionT<double> q_;
    MatrixMxNT<3, 3, double> R_;
    MatrixMxNT<4, 1, double> r_;
    MatrixMxNT<3, 1, double> p_;
    MatrixMxNT<6, 6, double> cov_;
} DevicePose;
...
typedef struct LocData {
    int type;
    CLVisionTime t;
    MatrixMxNT<3, 1, double> p_lla;
    MatrixMxNT<3, 1, double> p_ecef;
    CGFloat p_horz_acc;
    CGFloat p_vert_acc;
    CGFloat course_enu;
    ...
    CGFloat speed_acc;
    BOOL is_p_enu_valid;
    MatrixMxNT<3, 1, double> p_origin_lla;
    MatrixMxNT<3, 1, double> p_enu;
    MatrixMxNT<3, 3, double> p_enu_cov;
...
typedef struct LocationFuserState {
    ...
    MatrixMxNT<3, 1, double> p_origin_lla;
    MatrixMxNT<3, 1, double> p_origin_vio;
    LocData location;
    int status;
