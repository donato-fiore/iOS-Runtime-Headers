// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef NNMKPROTOCOMPOSEDMESSAGESENDINGPROGRESSREPORT_H
#define NNMKPROTOCOMPOSEDMESSAGESENDINGPROGRESSREPORT_H

@class PBCodable, NSString;
@protocol NSCopying;



@interface NNMKProtoComposedMessageSendingProgressReport : PBCodable <NSCopying>

 {
    ? _has;
}


@property (retain, nonatomic) NSString *composedMessageId; // ivar: _composedMessageId
@property (readonly, nonatomic) BOOL hasComposedMessageId;
@property (nonatomic) BOOL hasProgress;
@property (nonatomic) int progress; // ivar: _progress


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif