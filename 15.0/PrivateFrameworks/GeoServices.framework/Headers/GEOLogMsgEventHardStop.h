// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef GEOLOGMSGEVENTHARDSTOP_H
#define GEOLOGMSGEVENTHARDSTOP_H

@class PBCodable, PBDataReader, NSMutableArray;
@protocol NSCopying;



@interface GEOLogMsgEventHardStop : PBCodable <NSCopying>

 {
    PBDataReader *_reader;
    ? _digits;
    NSMutableArray *_cities;
    CGFloat _score;
    unsigned int _readerMarkPos;
    unsigned int _readerMarkLength;
    os_unfair_lock_s _readerLock;
    ? _flags;
}


@property (retain, nonatomic) NSMutableArray *cities;
@property (readonly, nonatomic) *unsigned int digits;
@property (readonly, nonatomic) NSUInteger digitsCount;
@property (nonatomic) BOOL hasScore;
@property (nonatomic) CGFloat score;


+(BOOL)isValid:(id)arg0 ;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)citiesAtIndex:(NSUInteger)arg0 ;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)init;
-(id)initWithData:(id)arg0 ;
-(id)initWithDictionary:(id)arg0 ;
-(id)initWithJSON:(id)arg0 ;
-(id)jsonRepresentation;
-(unsigned int)digitsAtIndex:(NSUInteger)arg0 ;
-(void)addCities:(id)arg0 ;
-(void)addDigits:(unsigned int)arg0 ;
-(void)clearCities;
-(void)clearDigits;
-(void)copyTo:(id)arg0 ;
-(void)dealloc;
-(void)mergeFrom:(id)arg0 ;
-(void)readAll:(BOOL)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif