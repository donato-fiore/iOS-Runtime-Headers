// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.0.0 | SDK: 15.0.0


#ifndef CASPAYMENTSHEETPRESENTATIONCONTEXT_H
#define CASPAYMENTSHEETPRESENTATIONCONTEXT_H

@class AMSDelegatePurchaseRequest, PKPaymentRequest, ACAccount;
@protocol NSSecureCoding;

#import <Foundation/Foundation.h>


@interface CASPaymentSheetPresentationContext : NSObject <NSSecureCoding>



@property (retain, nonatomic) AMSDelegatePurchaseRequest *delegatePurchaseRequest; // ivar: _delegatePurchaseRequest
@property (retain, nonatomic) PKPaymentRequest *localPaymentRequest; // ivar: _localPaymentRequest
@property (retain, nonatomic) ACAccount *storeAccount; // ivar: _storeAccount


+(BOOL)supportsSecureCoding;
-(id)description;
-(id)initWithCoder:(id)arg0 ;
-(void)encodeWithCoder:(id)arg0 ;


@end


#endif