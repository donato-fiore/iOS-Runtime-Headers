typedef struct _NSZone {
} // Error Processing Struct Fields

typedef struct ? {
    BOOL field0;
    BOOL field1;
    BOOL field2;
} ?;

typedef struct MPSKernelInfo {
} // Error Processing Struct Fields

typedef struct MPSDeviceSpecificInfo {
    MPSKernelInfo field0;
    unk field1;
    NSUInteger field2;
} MPSDeviceSpecificInfo;

typedef struct MPSLibraryInfo {
    int field0;
    unsigned int field1;
    char * field2;
    unk field3;
    MPSDeviceSpecificInfo field4;
    MPSDeviceSpecificInfo field5;
    MPSDeviceSpecificInfo field6;
    MPSDeviceSpecificInfo field7;
    MPSDeviceSpecificInfo field8;
    MPSDeviceSpecificInfo field9;
    MPSDeviceSpecificInfo field10;
    MPSDeviceSpecificInfo field11;
    MPSDeviceSpecificInfo field12;
    MPSDeviceSpecificInfo field13;
    MPSDeviceSpecificInfo field14;
    MPSDeviceSpecificInfo field15;
    MPSDeviceSpecificInfo field16;
    MPSDeviceSpecificInfo field17;
    MPSDeviceSpecificInfo field18;
    MPSDeviceSpecificInfo field19;
    MPSDeviceSpecificInfo field20;
    MPSDeviceSpecificInfo field21;
    MPSDeviceSpecificInfo field22;
    MPSDeviceSpecificInfo field23;
    MPSDeviceSpecificInfo field24;
    MPSDeviceSpecificInfo field25;
    MPSDeviceSpecificInfo field26;
    MPSDeviceSpecificInfo field27;
    MPSDeviceSpecificInfo field28;
    MPSDeviceSpecificInfo field29;
    MPSDeviceSpecificInfo field30;
    MPSDeviceSpecificInfo field31;
    MPSDeviceSpecificInfo field32;
    MPSDeviceSpecificInfo field33;
    MPSDeviceSpecificInfo field34;
    MPSDeviceSpecificInfo field35;
    MPSDeviceSpecificInfo field36;
    MPSDeviceSpecificInfo field37;
} MPSLibraryInfo;

typedef struct _MPSAxisAlignedBoundingBox {
} // Error Processing Struct Fields

typedef struct MPSBVHOptions {
    BOOL field0;
    int field1;
    float field2;
    float field3;
    unsigned int field4;
    unsigned int field5;
    unsigned int field6;
    unsigned int field7;
    unsigned int field8;
    unsigned int field9;
    int field10;
    unsigned int field11;
    int field12;
    int field13;
} MPSBVHOptions;

typedef struct MPSSVGFParams {
    float field0;
    float field1;
    float field2;
    float field3;
    float field4;
    unsigned int field5;
    float field6;
    float field7;
    float field8;
} MPSSVGFParams;

typedef struct MPSSVGFFunctionHash {
    unk field0;
    ? field1;
    NSUInteger field2;
    unsigned int field3;
    unsigned int field4;
    unsigned int field5;
} MPSSVGFFunctionHash;

typedef struct MPSTemporalAAParams {
    float field0;
} MPSTemporalAAParams;

typedef struct MPSTemporalAAFunctionHash {
    unk field0;
    ? field1;
    NSUInteger field2;
} MPSTemporalAAFunctionHash;

typedef struct MPSRayIntersectorFunctionHash {
    unk field0;
    ? field1;
    NSUInteger field2;
    unsigned int field3;
    unsigned int field4;
    unsigned int field5;
    unsigned int field6;
    unsigned int field7;
    unsigned int field8;
    unsigned int field9;
} MPSRayIntersectorFunctionHash;

