// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef PKPEERPAYMENTPENDINGREQUESTSREQUEST_H
#define PKPEERPAYMENTPENDINGREQUESTSREQUEST_H

@class NSArray;


#import "PKPeerPaymentWebServiceRequest.h"

@interface PKPeerPaymentPendingRequestsRequest : PKPeerPaymentWebServiceRequest

@property (copy, nonatomic) NSArray *requestTokens; // ivar: _requestTokens


-(id)_urlRequestWithServiceURL:(id)arg0 appleAccountInformation:(id)arg1 ;


@end


#endif