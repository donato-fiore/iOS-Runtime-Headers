// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef GEOPBTRANSITPAYMENTMETHODSUGGESTION_H
#define GEOPBTRANSITPAYMENTMETHODSUGGESTION_H

@class PBCodable, PBDataReader, PBUnknownFields;
@protocol NSCopying;


#import "GEOPBTransitPaymentMethodSuggestionDetails.h"

@interface GEOPBTransitPaymentMethodSuggestion : PBCodable <NSCopying>

 {
    PBDataReader *_reader;
    PBUnknownFields *_unknownFields;
    ? _paymentMethodIndexs;
    GEOPBTransitPaymentMethodSuggestionDetails *_paymentMethodSuggestionDetails;
    unsigned int _readerMarkPos;
    unsigned int _readerMarkLength;
    os_unfair_lock_s _readerLock;
    int _purpose;
    int _type;
    ? _flags;
}


@property (readonly, nonatomic) BOOL hasPaymentMethodSuggestionDetails;
@property (nonatomic) BOOL hasPurpose;
@property (nonatomic) BOOL hasType;
@property (readonly, nonatomic) *unsigned int paymentMethodIndexs;
@property (readonly, nonatomic) NSUInteger paymentMethodIndexsCount;
@property (retain, nonatomic) GEOPBTransitPaymentMethodSuggestionDetails *paymentMethodSuggestionDetails;
@property (nonatomic) int purpose;
@property (nonatomic) int type;
@property (readonly, nonatomic) PBUnknownFields *unknownFields;


+(BOOL)isValid:(id)arg0 ;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)init;
-(id)initWithData:(id)arg0 ;
-(id)initWithDictionary:(id)arg0 ;
-(id)initWithJSON:(id)arg0 ;
-(id)jsonRepresentation;
-(id)purposeAsString:(int)arg0 ;
-(id)typeAsString:(int)arg0 ;
-(int)StringAsPurpose:(id)arg0 ;
-(int)StringAsType:(id)arg0 ;
-(unsigned int)paymentMethodIndexAtIndex:(NSUInteger)arg0 ;
-(void)addPaymentMethodIndex:(unsigned int)arg0 ;
-(void)clearPaymentMethodIndexs;
-(void)clearUnknownFields:(BOOL)arg0 ;
-(void)copyTo:(id)arg0 ;
-(void)dealloc;
-(void)mergeFrom:(id)arg0 ;
-(void)readAll:(BOOL)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif