// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef BLSHASSERTIONATTRIBUTETIMER_H
#define BLSHASSERTIONATTRIBUTETIMER_H

@class BLSDurationAttribute, NSString;
@protocol BLSHLocalAssertionAttributeHandlerEntry, BSInvalidatable, BSCancellable, BLSAssertionServiceResponding, BLSHAssertionAttributeHandlerService;

#import <Foundation/Foundation.h>


@interface BLSHAssertionAttributeTimer : NSObject <BLSHLocalAssertionAttributeHandlerEntry, BSInvalidatable, BSCancellable>

 {
    os_unfair_lock_s _lock;
    id<BSInvalidatable> *_timer;
    BOOL _invalidated;
}


@property (readonly, weak, nonatomic) NSObject<BLSAssertionServiceResponding> *assertion; // ivar: _assertion
@property (readonly, weak, nonatomic) BLSDurationAttribute *attribute; // ivar: _attribute
@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (readonly) NSUInteger hash;
@property (readonly, weak, nonatomic) NSObject<BLSHAssertionAttributeHandlerService> *service; // ivar: _service
@property (readonly) Class superclass;


+(id)activateForAttribute:(id)arg0 fromAssertion:(id)arg1 forService:(id)arg2 attributeHandler:(id)arg3 ;
-(BOOL)reactivateIfPossible;
-(id)initForAttribute:(id)arg0 fromAssertion:(id)arg1 forService:(id)arg2 ;
-(void)cancel;
-(void)dealloc;
-(void)invalidate;
-(void)timerFired;


@end


#endif