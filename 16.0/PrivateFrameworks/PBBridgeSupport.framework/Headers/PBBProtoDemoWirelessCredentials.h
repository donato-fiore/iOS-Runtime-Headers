// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef PBBPROTODEMOWIRELESSCREDENTIALS_H
#define PBBPROTODEMOWIRELESSCREDENTIALS_H

@class PBCodable, NSString;
@protocol NSCopying;



@interface PBBProtoDemoWirelessCredentials : PBCodable <NSCopying>

 {
    ? _has;
}


@property (nonatomic) unsigned int companionDemoConfig; // ivar: _companionDemoConfig
@property (nonatomic) BOOL hasCompanionDemoConfig;
@property (readonly, nonatomic) BOOL hasPassword;
@property (readonly, nonatomic) BOOL hasSsid;
@property (retain, nonatomic) NSString *password; // ivar: _password
@property (retain, nonatomic) NSString *ssid; // ivar: _ssid


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif