// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef _MRVOICEINPUTDEVICE_H
#define _MRVOICEINPUTDEVICE_H

@class PBCodable;
@protocol NSCopying;


#import "_MRVoiceInputDeviceDescriptorProtobuf.h"

@interface _MRVoiceInputDevice : PBCodable <NSCopying>

 {
    ? _has;
}


@property (retain, nonatomic) _MRVoiceInputDeviceDescriptorProtobuf *descriptor; // ivar: _descriptor
@property (nonatomic) unsigned int deviceID; // ivar: _deviceID
@property (readonly, nonatomic) BOOL hasDescriptor;
@property (nonatomic) BOOL hasDeviceID;
@property (nonatomic) BOOL hasRecordingState;
@property (nonatomic) int recordingState; // ivar: _recordingState


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)recordingStateAsString:(int)arg0 ;
-(int)StringAsRecordingState:(id)arg0 ;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif