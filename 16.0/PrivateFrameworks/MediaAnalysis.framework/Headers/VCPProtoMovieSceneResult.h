// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef VCPPROTOMOVIESCENERESULT_H
#define VCPPROTOMOVIESCENERESULT_H

@class PBCodable;
@protocol VCPProtoResultLegacyConversionProtocol, NSCopying;


#import "VCPProtoTimeRange.h"

@interface VCPProtoMovieSceneResult : PBCodable <VCPProtoResultLegacyConversionProtocol, NSCopying>

 {
    ? _has;
}


@property (nonatomic) float distanceToPreviousScene; // ivar: _distanceToPreviousScene
@property (nonatomic) float flickerScore; // ivar: _flickerScore
@property (nonatomic) BOOL hasDistanceToPreviousScene;
@property (nonatomic) BOOL hasFlickerScore;
@property (nonatomic) BOOL hasSceneprintDistanceToPreviousScene;
@property (nonatomic) float qualityScore; // ivar: _qualityScore
@property (nonatomic) float sceneprintDistanceToPreviousScene; // ivar: _sceneprintDistanceToPreviousScene
@property (retain, nonatomic) VCPProtoTimeRange *timeRange; // ivar: _timeRange


+(id)resultFromLegacyDictionary:(id)arg0 ;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)exportToLegacyDictionary;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif