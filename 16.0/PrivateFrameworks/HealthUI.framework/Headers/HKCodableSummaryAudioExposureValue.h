// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef HKCODABLESUMMARYAUDIOEXPOSUREVALUE_H
#define HKCODABLESUMMARYAUDIOEXPOSUREVALUE_H

@class PBCodable;
@protocol NSCopying;


#import "HKCodableQuantity.h"

@interface HKCodableSummaryAudioExposureValue : PBCodable <NSCopying>

 {
    APPLE_144 _has;
}


@property (nonatomic) NSInteger daysAggregated; // ivar: _daysAggregated
@property (nonatomic) BOOL hasDaysAggregated;
@property (nonatomic) BOOL hasLastUpdatedDateData;
@property (readonly, nonatomic) BOOL hasLeqQuantity;
@property (nonatomic) BOOL hasSecondsListened;
@property (nonatomic) CGFloat lastUpdatedDateData; // ivar: _lastUpdatedDateData
@property (retain, nonatomic) HKCodableQuantity *leqQuantity; // ivar: _leqQuantity
@property (nonatomic) CGFloat secondsListened; // ivar: _secondsListened


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif