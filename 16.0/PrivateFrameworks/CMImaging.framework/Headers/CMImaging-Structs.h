typedef struct CGPoint {
    CGFloat field0;
    CGFloat field1;
} CGPoint;

typedef struct CGSize {
    CGFloat field0;
    CGFloat field1;
} CGSize;

typedef struct CGRect {
    CGPoint field0;
    CGSize field1;
} CGRect;

typedef struct _NSZone {
} // Error Processing Struct Fields

typedef struct ? {
    float pixelPitch;
    float distortionCenter;
    float inverseScalingLUTBinSizeRecip;
    float radiusScalingNormFactor;
} ?;

typedef struct CMIWarpConfig {
} // Error Processing Struct Fields

typedef struct __CVBuffer {
} // Error Processing Struct Fields

typedef struct __IOSurface {
} // Error Processing Struct Fields

typedef struct MTLTargetDeviceArch {
    NSUInteger field0;
    unsigned int field1;
    char * field2;
} MTLTargetDeviceArch;

typedef struct IndirectArgumentBufferCapabilities {
    BOOL field0;
    BOOL field1;
    BOOL field2;
    BOOL field3;
} IndirectArgumentBufferCapabilities;

typedef struct os_unfair_lock_s {
    unsigned int field0;
} os_unfair_lock_s;

typedef struct MPSFunctionTable {
} // Error Processing Struct Fields

typedef struct _NSRange {
    NSUInteger field0;
    NSUInteger field1;
} _NSRange;

typedef struct MTLRangeAllocatorElement {
} // Error Processing Struct Fields

typedef struct MTLRangeAllocator {
    MTLRangeAllocatorElement elements;
    unsigned int numElements;
    NSUInteger capacity;
    NSUInteger capacityIncrement;
    NSUInteger defaultAlignmentMask;
} MTLRangeAllocator;

typedef struct MTLResourceID {
    NSUInteger field0;
} MTLResourceID;

typedef struct __IOSurfaceAccelerator {
} // Error Processing Struct Fields

typedef struct __CFDictionary {
} // Error Processing Struct Fields

typedef struct rwppRegistrationEngineS {
    ? field0;
    ? field1;
    ? field2;
    BOOL field3;
    unsigned int field4;
    unsigned int field5;
    ? field6;
    unsigned int field7;
    unsigned int field8;
    ? field9;
    ? field10;
    ? field11;
    float field12;
    ? field13;
    ? field14;
    ? field15;
    id field16;
    id field17;
} rwppRegistrationEngineS;

