// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef CLPINSTSCHEMAHABITUALUSAGEBYCALENDARWEEKINFORMATION_H
#define CLPINSTSCHEMAHABITUALUSAGEBYCALENDARWEEKINFORMATION_H

@class NSData;


#import "SISchemaInstrumentationMessage.h"

@interface CLPInstSchemaHabitualUsageByCalendarWeekInformation : SISchemaInstrumentationMessage {
    ? _has;
}


@property (nonatomic) unsigned int calendarWeekNumber; // ivar: _calendarWeekNumber
@property (nonatomic) BOOL hasCalendarWeekNumber;
@property (nonatomic) BOOL hasWasHabitualUserDuringCalendarWeek;
@property (readonly, nonatomic) NSData *jsonData;
@property (nonatomic) BOOL wasHabitualUserDuringCalendarWeek; // ivar: _wasHabitualUserDuringCalendarWeek


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)dictionaryRepresentation;
-(id)initWithDictionary:(id)arg0 ;
-(id)initWithJSON:(id)arg0 ;
-(id)suppressMessageUnderConditions;
-(void)deleteCalendarWeekNumber;
-(void)deleteWasHabitualUserDuringCalendarWeek;
-(void)writeTo:(id)arg0 ;


@end


#endif