// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef PKTOOL_H
#define PKTOOL_H

@protocol NSCopying;

#import <Foundation/Foundation.h>

#import "PKToolConfiguration.h"
#import "PKInk.h"

@interface PKTool : NSObject <NSCopying>



@property (readonly, nonatomic) PKToolConfiguration *_configuration; // ivar: _configuration
@property (readonly, nonatomic) PKInk *ink; // ivar: _ink


+(BOOL)_inkIsInkingTool:(id)arg0 ;
+(id)_toolWithInk:(id)arg0 ;
+(id)_toolWithInk:(id)arg0 configuration:(id)arg1 ;
-(BOOL)isEqual:(id)arg0 ;
-(NSUInteger)hash;
-(id)_initWithInk:(id)arg0 ;
-(id)_initWithInk:(id)arg0 configuration:(id)arg1 ;
-(id)copyWithZone:(struct _NSZone *)arg0 ;


@end


#endif