// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 16.0.0 | SDK: 16.0.0


#ifndef _INPBRIDEPARTYSIZEOPTION_H
#define _INPBRIDEPARTYSIZEOPTION_H

@class PBCodable, NSString;
@protocol _INPBRidePartySizeOption, NSSecureCoding, NSCopying;


#import "_INPBRangeValue.h"
#import "_INPBPriceRangeValue.h"

@interface _INPBRidePartySizeOption : PBCodable <_INPBRidePartySizeOption, NSSecureCoding, NSCopying>

 {
    ? _has;
}


@property (readonly, copy) NSString *debugDescription;
@property (readonly, copy) NSString *description;
@property (readonly, nonatomic) BOOL hasPartySizeRange;
@property (readonly, nonatomic) BOOL hasPriceRange;
@property (readonly, nonatomic) BOOL hasSizeDescription;
@property (readonly) NSUInteger hash;
@property (retain, nonatomic) _INPBRangeValue *partySizeRange; // ivar: _partySizeRange
@property (retain, nonatomic) _INPBPriceRangeValue *priceRange; // ivar: _priceRange
@property (copy, nonatomic) NSString *sizeDescription; // ivar: _sizeDescription
@property (readonly) Class superclass;


+(BOOL)supportsSecureCoding;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)dictionaryRepresentation;
-(id)initWithCoder:(id)arg0 ;
-(void)encodeWithCoder:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif