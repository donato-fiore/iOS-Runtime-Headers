// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.6.0 | SDK: 15.6.0


#ifndef BLTPBCONTACT_H
#define BLTPBCONTACT_H

@class PBCodable, NSString;
@protocol NSCopying;



@interface BLTPBContact : PBCodable <NSCopying>

 {
    ? _has;
}


@property (retain, nonatomic) NSString *cnContactFullname; // ivar: _cnContactFullname
@property (retain, nonatomic) NSString *cnContactIdentifier; // ivar: _cnContactIdentifier
@property (nonatomic) BOOL cnContactIdentifierSuggested; // ivar: _cnContactIdentifierSuggested
@property (retain, nonatomic) NSString *customIdentifier; // ivar: _customIdentifier
@property (retain, nonatomic) NSString *displayName; // ivar: _displayName
@property (nonatomic) BOOL displayNameSuggested; // ivar: _displayNameSuggested
@property (retain, nonatomic) NSString *handle; // ivar: _handle
@property (nonatomic) unsigned int handleType; // ivar: _handleType
@property (readonly, nonatomic) BOOL hasCnContactFullname;
@property (readonly, nonatomic) BOOL hasCnContactIdentifier;
@property (readonly, nonatomic) BOOL hasCustomIdentifier;
@property (readonly, nonatomic) BOOL hasDisplayName;
@property (nonatomic) BOOL hasDisplayNameSuggested;
@property (readonly, nonatomic) BOOL hasHandle;
@property (readonly, nonatomic) BOOL hasServiceName;
@property (retain, nonatomic) NSString *serviceName; // ivar: _serviceName


-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(void)copyTo:(id)arg0 ;
-(void)mergeFrom:(id)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif