// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.6.0 | SDK: 15.6.0


#ifndef GEOLPRREGIONINFO_H
#define GEOLPRREGIONINFO_H

@class PBCodable, PBDataReader, NSString, NSMutableArray;
@protocol NSCopying;



@interface GEOLPRRegionInfo : PBCodable <NSCopying>

 {
    PBDataReader *_reader;
    ? _routingRequiredFields;
    NSString *_licensePlateTemplate;
    NSMutableArray *_licensePlateValidationRules;
    NSMutableArray *_validPowerTypeKeys;
    NSMutableArray *_validVehicleTypeKeys;
    unsigned int _readerMarkPos;
    unsigned int _readerMarkLength;
    os_unfair_lock_s _readerLock;
    ? _flags;
}


@property (readonly, nonatomic) BOOL hasLicensePlateTemplate;
@property (retain, nonatomic) NSString *licensePlateTemplate;
@property (retain, nonatomic) NSMutableArray *licensePlateValidationRules;
@property (readonly, nonatomic) *int routingRequiredFields;
@property (readonly, nonatomic) NSUInteger routingRequiredFieldsCount;
@property (retain, nonatomic) NSMutableArray *validPowerTypeKeys;
@property (retain, nonatomic) NSMutableArray *validVehicleTypeKeys;


+(BOOL)isValid:(id)arg0 ;
-(BOOL)isEqual:(id)arg0 ;
-(BOOL)readFrom:(id)arg0 ;
-(NSUInteger)hash;
-(id)copyWithZone:(struct _NSZone *)arg0 ;
-(id)description;
-(id)dictionaryRepresentation;
-(id)init;
-(id)initWithData:(id)arg0 ;
-(id)initWithDictionary:(id)arg0 ;
-(id)initWithJSON:(id)arg0 ;
-(id)jsonRepresentation;
-(id)licensePlateValidationRulesAtIndex:(NSUInteger)arg0 ;
-(id)routingRequiredFieldsAsString:(int)arg0 ;
-(id)validPowerTypeKeysAtIndex:(NSUInteger)arg0 ;
-(id)validVehicleTypeKeysAtIndex:(NSUInteger)arg0 ;
-(int)StringAsRoutingRequiredFields:(id)arg0 ;
-(int)routingRequiredFieldsAtIndex:(NSUInteger)arg0 ;
-(void)addLicensePlateValidationRules:(id)arg0 ;
-(void)addRoutingRequiredFields:(int)arg0 ;
-(void)addValidPowerTypeKeys:(id)arg0 ;
-(void)addValidVehicleTypeKeys:(id)arg0 ;
-(void)clearLicensePlateValidationRules;
-(void)clearRoutingRequiredFields;
-(void)clearValidPowerTypeKeys;
-(void)clearValidVehicleTypeKeys;
-(void)copyTo:(id)arg0 ;
-(void)dealloc;
-(void)mergeFrom:(id)arg0 ;
-(void)readAll:(BOOL)arg0 ;
-(void)writeTo:(id)arg0 ;


@end


#endif