// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.6.0 | SDK: 15.6.0


#ifndef WBSCYCLEROPERATIONCONTEXT_H
#define WBSCYCLEROPERATIONCONTEXT_H

@class NSString;
@protocol WBSCyclerTestTarget;

#import <Foundation/Foundation.h>

#import "WBSCyclerItemListRepresentation.h"

@interface WBSCyclerOperationContext : NSObject

@property (retain, nonatomic) NSObject<WBSCyclerTestTarget> *testTarget; // ivar: _testTarget
@property (copy, nonatomic) NSString *titlePrefix; // ivar: _titlePrefix
@property (retain, nonatomic) WBSCyclerItemListRepresentation *topLevelItem; // ivar: _topLevelItem


-(BOOL)_isItemPrefixed:(id)arg0 ;
-(BOOL)_isItemValid:(id)arg0 ;
-(id)_buildDictionaryOfDescendantIdentifiersToParentIdentifiersFromList:(id)arg0 ;
-(id)_copyItem:(id)arg0 filteringWithIdentifierWhitelist:(id)arg1 ;
-(id)filterOutItemsWithoutTitlePrefixInList:(id)arg0 ;
-(id)randomDescendantOfList:(id)arg0 enforcingTitlePrefixValidity:(BOOL)arg1 passingTest:(id)arg2 ;
-(id)randomItemTitle;
-(id)randomValidStrictDescendantOfList:(id)arg0 ;
-(id)randomValidStrictDescendantOfTopLevelItem;
-(id)randomValidStrictListDescendantOfList:(id)arg0 ;
-(id)randomValidStrictListDescendantOfTopLevelItem;


@end


#endif