// Headers generated with ktool v2.0.0
// https://github.com/cxnder/ktool | pip3 install k2l
// Platform: IOS | Minimum OS: 15.6.0 | SDK: 15.6.0


#ifndef CHIPTESTOTASOFTWAREUPDATEPROVIDER_H
#define CHIPTESTOTASOFTWAREUPDATEPROVIDER_H



#import "CHIPOtaSoftwareUpdateProvider.h"

@interface CHIPTestOtaSoftwareUpdateProvider : CHIPOtaSoftwareUpdateProvider {
    OtaSoftwareUpdateProviderClusterTest _cppCluster;
}


@property ? cppCluster;


-(void)writeAttributeAttributeListWithValue:(id)arg0 completionHandler:(id)arg1 ;
-(void)writeAttributeClusterRevisionWithValue:(id)arg0 completionHandler:(id)arg1 ;


@end


#endif